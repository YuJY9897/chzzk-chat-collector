...
치지직 방송 채팅을 모아두고, **편집할 구간(클립각)을 채팅이 몰린 지점으로 찾아주는** 윈도우 데스크톱 앱입니다. 앱 이름은 `CHZZK Clip Scout`입니다.
...
![분석 화면](docs/screenshot-analyze.png)

> 화면의 채팅은 `tools/make-dummy-log.mjs`로 만든 더미 데이터입니다.

## 3단계로 씁니다

1. **수집** — 방송 전에 켜두면 채팅이 시각과 함께 CSV/JSONL로 쌓입니다
2. **분석** — 저장된 로그를 고르면 채팅이 몰린 구간을 찾아 줍니다. 방송 중이 아니어도 언제든 됩니다
3. **편집** — 구간 시각을 복사해 다시보기에서 그 지점만 보고 자릅니다

분석 결과는 이렇게 나옵니다.

```text
7,396채팅 · 1:29:58 · 발화자 330명 · 분당 82.2개

하이라이트 5개  — 누르면 그 구간 채팅을 봅니다
 1  28:40 ~ 29:50  [놀람]  70초 · 분당 76→984개 · 채팅 651줄
    미쳤다  방금  뒤에
    ㅋㅋㅋㅋㅋㅋㅋㅋㅋ x35 · 헐 x34 · 이걸 성공하네 x28
 2  43:40 ~ 45:05  [놀람]  85초 · 분당 64→720개 · 채팅 662줄
    ㅋㅋㅋㅋㅋㅋ x45 · 와 진짜? x30 · 갓유자주 x28

발화자 330명 — 상위 10%가 전체의 46%, 1줄만 친 사람 6명
 1 초코초보73    ████████████████████  196줄 3%
 2 고슴도치헌터  ███████████████████░  194줄 3%
```
...
> **채팅을 수집하고, 수집이 끝나면 채팅이 몰린 구간을 찾아 편집점 후보로 알려주는** 도구입니다.
...
![수집 화면](docs/screenshot-collect.png)
...
- **편집점 후보를 자동으로 뽑습니다** — 채팅량 폭증 + 반응성 표현 비율 + 참여자 수 급증을 함께 보고, 기준선을 앞뒤 10분 중앙값으로 잡아 시청자가 늘고 줄어도 따라가게 했습니다. 정답지를 심은 더미 로그 생성기를 만들어 재현율/정밀도를 채점할 수 있게 했습니다
- **개발 환경 없이도 쓸 수 있게** — Electron으로 exe 빌드, 트레이 상주 백그라운드 수집, 종료 시 저장 위치와 하이라이트를 알림으로 안내
- **API의 한계를 문서로 남겼습니다** — 방송 종료를 자동 감지할 수 있는지 공식 API를 하나씩 실측해 확인했고(`revoked` 이벤트 미수신, 구독은 방송 여부와 무관하게 성공 등), 안 되는 이유와 근거를 README에 적어 다음 사람이 같은 시도를 반복하지 않게 했습니다
- **범위를 처음부터 제한했습니다** — 공식 API 권한을 허용한 채널의 라이브 채팅만 대상으로 하고, `clientSecret` 등 민감 정보는 `.env`로 분리해 저장소에 올리지 않습니다

## 비슷한 서비스와의 차이 — 공식 API만 쓰기로 한 이유

채팅 반응으로 편집점을 찾는다는 발상은 이미 서비스로 존재합니다(Captus, ClipSpray 등). 그쪽은 **다시보기 링크 하나만 넣으면** 지나간 방송의 채팅까지 분석해 줍니다.

그런데 **치지직 공식 Open API에는 다시보기 채팅을 가져오는 기능이 없습니다.** Live·Session·Chat API를 모두 확인했고, 공식 API로 얻을 수 있는 채팅은 **라이브 중 실시간으로 흘러오는 이벤트뿐**입니다. 비공식 라이브러리들이 "공식 API로는 채팅 이력을 못 가져와서" 문서화되지 않은 내부 엔드포인트를 병행 지원한다고 밝히고 있는 것도 같은 이유입니다.

이 프로젝트는 **공식 Open API와 OAuth 2.0 동의 절차만 사용합니다.** 비공식 내부 API 호출, 쿠키·세션 재사용, 다시보기 페이지 크롤링을 하지 않습니다. 네이버 이용약관이 자동화된 수단을 통한 무단 수집을 금지하고 있고, 비공식 경로는 예고 없는 차단이나 계정 보호 조치의 위험이 따르기 때문입니다.

그래서 이런 차이가 생깁니다.

| | 다시보기 기반 도구 | 이 프로젝트 |
|---|---|---|
| 지나간 방송 | 분석 가능 | **불가능** (미리 켜둬야 함) |
| 사용 경로 | 공개된 정보 없음 | 공식 Open API + OAuth 동의 |
| 채널 접근 | 링크만 있으면 됨 | 채널 소유자가 직접 권한에 동의해야 함 |
| 정책 변경 영향 | 알 수 없음 | 공식 API 스펙을 따름 |
| 원본 데이터 | 서비스가 보관 | 내 PC에 CSV/JSONL로 남음 |

다른 서비스가 어떤 방법을 쓰는지는 공개된 정보가 없어 여기서 단정하지 않습니다. 확인할 수 있는 것은 공식 API에 그 기능이 없다는 사실과, 이 프로젝트가 무엇을 쓰고 무엇을 쓰지 않았는지뿐입니다.

"지나간 방송을 못 쓴다"는 건 명백한 불편입니다. 그 불편을 감수하는 대신 **허용된 방법만으로 만든다**는 쪽을 택했습니다.
...
8. 앱을 완전히 끄려면 화면 맨 아래 `앱 종료` 버튼을 누르거나, 트레이 아이콘 우클릭 → `앱 종료`를 누릅니다
...
앱은 **수집**과 **분석** 두 화면으로 나뉩니다.

- **수집** — 방송 중에 채팅을 저장합니다. 현재 상태와 수집 ON/일시정지/종료 버튼이 있고, 치지직 연결·저장 설정·실시간 채팅은 접이식으로 펼쳐 봅니다
- **분석** — 저장해 둔 로그를 골라 언제든 분석합니다. 방송 중이 아니어도 됩니다
  - 감도(민감하게 / 보통 / 엄격하게)를 바꿔 다시 분석할 수 있습니다
  - 구간마다 성격 라벨(웃음 / 놀람 / 클립 요청 / 감탄 / 안타까움)과 특징 키워드를 보여줍니다. 규칙 기반이라 외부 API나 비용이 필요 없습니다
  - 하이라이트를 누르면 그 구간의 채팅 원문이 펼쳐집니다
  - 발화자 통계: 상위 10명의 채팅 수와 비중, 상위 10%의 쏠림 정도, 한 줄만 친 사람 수
  - 구간별 채팅량: 1·5·10·30분 간격으로 방송 전체를 잘라 어느 시간대가 활발했는지 봅니다. 하이라이트가 놓친 구간도 사람이 직접 확인할 수 있습니다
  - `다시보기 댓글용 타임스탬프 복사`로 시각 목록을 복사할 수 있습니다. 치지직 VOD URL에는 시간 파라미터가 없지만, 다시보기 **댓글에 적은 시각은 눌러서 이동**할 수 있어 그 형식에 맞춰 만듭니다
  - `방송 기록 저장`으로 편집자에게 넘길 문서(`*.report.md`)를 원본 로그 옆에 만듭니다. 편집점 후보가 **시간순**으로 정리되고 구간별 채팅량과 발화자 요약이 함께 담깁니다
- **정보** — 사용 설명서, 패치노트(버전별 추가·수정), 고지사항, 피드백 보내기
...
이 도구의 출발점은 "방송을 보면서 재미있던 장면·반응 좋았던 장면·무엇을 했는지 타임라인으로 기록"하던 사람의 일을 대신하는 것입니다. 그래서 결과를 화면에만 두지 않고 **편집자에게 그대로 넘길 수 있는 문서**로 저장합니다.

분석 탭에서 `방송 기록 저장`을 누르면 원본 로그 옆에 `*.report.md`가 만들어집니다.

```markdown
# 방송 기록 — 20260908_...

- 방송 길이: 01:29:59
- 총 채팅: 8,527개 (분당 평균 94.8개)
- 발화자: 330명 (상위 10%가 전체의 44%)

## 편집점 후보 5개

### 1. 00:07:40 ~ 00:09:35 · 놀람
- 반응: 분당 72 → 724개, 이 구간 채팅 1,000줄
- 키워드: 미쳤다, 방금, 뒤에
- 반응 예시: ㅋㅋㅋㅋㅋㅋ (52회) · 대박 (39회) · 이거 클립각 (37회)
